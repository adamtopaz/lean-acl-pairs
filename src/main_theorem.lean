...
begin
  ...
  { have hvu : v ∈ R.principal_units, 
    { split, convert R.neg_mem huR, rwa [sub_eq_iff_eq_add, eq_neg_add_iff_add_eq],
  ...
  { have huu : (u : K)⁻¹ ∈ R, 
    ...
    { split, push_cast, rw ← neg_add', apply R.neg_mem, 
      ...
      convert R.mul_mem huu this, field_simp, ring,
      intro c, apply huR, 
      ...
      convert R.mul_mem this hw1.2, field_simp [w.ne_zero], ring },
...
begin
  ...
  { convert hacl },
  have := main_theorem_mul T hacl',
  obtain ⟨R,H,le,units,principal_units,fd,codim⟩ := this,
  let I := H.dual_annihilator,
  have : nonempty ((↥D ⧸ submodule.comap D.subtype I) ≃ₗ[F] 
    (dual F (↥H ⧸ T.comap H.subtype))), 
  { dsimp [I],
    rw ← hTD,
    have := submodule.dual_mod_comap_iso T H le,
    use this },
  obtain ⟨e⟩ := this,
