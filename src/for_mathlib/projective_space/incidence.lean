...
begin
  ...
  { intro h, rw [dependent_iff_rep, fintype.linear_independent_iff] at h, 
    ...
    simp only [function.comp_app, fin.sum_univ_two, matrix.cons_val_zero, 
...
begin
  ...
  { simp only [fin.sum_univ_succ, function.comp_app, matrix.cons_val_zero, matrix.cons_val_succ, 
      fintype.univ_of_subsingleton, fin.mk_zero, fin.succ_zero_eq_one, finset.sum_singleton, 
      fin.succ_one_eq_two] at *,
...
begin
  ...
  { intros h, 
    ...
    obtain ⟨g,h1,i,hi⟩ := h,
    simp only [fin.sum_univ_succ, function.comp_app, matrix.cons_val_zero, matrix.cons_val_succ, 
      fin.succ_zero_eq_one, fintype.univ_of_subsingleton, fin.mk_zero, finset.sum_singleton, 
